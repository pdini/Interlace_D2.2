...
\vspace{0.3cm}
$*$Let $U_a$ and $U_b$ be two quorums in $(V, Q)^B = U_1 \cup U_2$:
\vspace{0.3cm}
\begin{packed_item1}
...
This implies that there are now three possibilities:
\vspace{0.3cm}
...
\vspace{0.3cm}
If 1) is true, then $(U_a \setminus B_1) \cap U \ne \emptyset$, by QI of $(V, Q)^{B_1}$.\footnote{$(V, Q)^{B_1}$ has QI because $B_1$ is a DSet. And $B_1$ is a DSet by construction.} Let us revisit why:
...
$U_a \setminus B_2 \ne \emptyset$. Therefore, $U_a \setminus B_2$ is a quorum in $(V, Q)^{B_2}$.

\vspace{0.3cm}
[Similarly, if 2) is true, then $U_a \setminus B_1$ is a quorum in $(V, Q)^{B_1}$.]

\vspace{0.3cm}
Now go back to $*$ and follow the same argument for 1) $U_b \setminus B_1$, 2) etc, reaching the conclusion for case 1) that $U_b \setminus B_2$ is a quorum in $(V, Q)^{B_2}$. But then by QI despite $B_2$
\begin{align}
(U_a \setminus B_2) \cap (U_b \setminus B_2) \ne \emptyset.
\end{align}
Hence, $U_a \cap U_b \ne \emptyset$. Since $U_a$ and $U_b$ are any two quorums in $(V, Q)^B$, we have proven QI despite $B$. Hence $B$ is a DSET.
\end{case}
\vspace{-0.5cm}
\end{proof}
\end{quote}
