...
\end{defin}
Therefore, when a FBAS has many quorums, quorum intersection (QI) fails when \emph{any two} do not intersect.

We remind the reader that $Q(v_i)$ is just the set of slices of a given node $v_i$ and, depending on the slices of all the other nodes $v_j \in Q(v_i)$, may or may not be a quorum $U \subseteq V$. 
